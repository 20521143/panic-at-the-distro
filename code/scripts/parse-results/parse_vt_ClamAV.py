...
def save_results_to_csv(results, output_file):
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['Package', 'Dataset', '#AVs', 'ClamAV']
        ...
        for result in results:
            ...
            # for av in av_names:
            av = 'ClamAV'
            flaged_av = result.get('av_results', {}).get(av, {}).get('category', 'unknown')
            if flaged_av == 'unknown':
                print(f"Unknown ClamAV result for {result['package']}")

            flagged_set.add(flaged_av)
        ...
        print(f"Flagged ClamAV results: {flagged_set}")
